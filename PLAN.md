...
| ID | パターン | 主な物証 |
|----|---------|---------|
| P1 | Process() 直接生成が 5 ファイル・約 80 箇所に散在。DI なし・テスト困難 | TmuxProvider / ProcessProvider / AppleScriptBridge / focusbm.swift / FocusBMApp.swift |
...
| P3 | log() が TmuxProvider:185 と ProcessProvider:83 の 2 箇所に private static で重複定義。絵文字辞書が 4 ファイル散在 | TmuxProvider.swift / ProcessProvider.swift / Models.swift / BookmarkRow.swift |
| P4 | SearchView で LazyVStack / LazyVGrid の ForEach ブロック 35 行×2 が重複 | SearchView.swift:49-122 |
| P5 | AppDelegate が setupHotkey/setupStatusItem/setupSearchPanel で YAMLStorage.loadYAML() を 3 重呼出（Phase3 で SearchViewModel 側は修正済みだが AppDelegate 側は残存） | FocusBMApp.swift:38-256 |
...
- `Process()` 直接使用: TmuxProvider / ProcessProvider / AppleScriptBridge / focusbm.swift / FocusBMApp.swift（5 ファイル・約 80 箇所）
- `log()` 独自定義: TmuxProvider.swift:185、ProcessProvider.swift:83（各型が private static func log を保有）
- 絵文字辞書: TmuxProvider / Models / ProcessProvider / BookmarkRow の 4 ファイル散在
- YAML 読み込み: FocusBMApp.swift（3 箇所）/ YAMLStorage.swift（正常）
...
**touched_files**:
- `Sources/FocusBMLib/TmuxProvider.swift`（log 定義削除・絵文字参照置換）
- `Sources/FocusBMLib/ProcessProvider.swift`（log 定義削除・絵文字参照置換）
- `Sources/FocusBMApp/BookmarkRow.swift`（絵文字参照置換）
- `Sources/FocusBMLib/Models.swift`（絵文字参照置換）
- `Sources/FocusBMLib/(新規) EmojiCatalog.swift`（絵文字辞書を単一定義）
- `Sources/FocusBMLib/(新規) DebugLog.swift`（log() を単一定義）

**scope_constraint**: touched_lines <= 250（純粋データ移動のみ。検出ロジック本体は触らない）
...
#### Green
1. `EmojiCatalog.swift` に絵文字辞書 3 箇所→1 箇所集約
...
**anti_pattern_guard**: 絵文字/log の全箇所を横断 grep してから着手（既確認: 絵文字 4 ファイル・log 2 定義）
...
**Waste**: P1（Process() 直接生成 5 ファイル・80 箇所）
...
**scope_constraint**: touched_lines <= 400（80 箇所置換のため上限緩和。1 コミット = 1 ファイル厳守 [M1]）
...
**Waste**: P2（TmuxProvider God 型 848 行）/ P3（ProcessProvider→TmuxProvider 逆方向依存）

**touched_files**:
- `Sources/FocusBMLib/TmuxProvider.swift`（分割元・削除）
- `Sources/FocusBMLib/ProcessProvider.swift`（逆依存解消）
- `Sources/FocusBMLib/(新規) TerminalDetector.swift`（TTY/親子 PID/作業ディレクトリ解決を集約）
...
**staff-validation**: 必須（Cynefin=Complex・逆方向依存あり）
...
- ProcessProvider→TmuxProvider 逆方向依存を TerminalDetector へ反転集約
...
**Waste**: P2（SearchViewModel God 型 434 行・AppDelegate 353 行）/ P5（YAML 3 重読込）
...
- `setupHotkey` / `setupStatusItem` / `setupSearchPanel` で個別呼出している `loadYAML()` を起動時 1 回の単一呼出に集約
...
**anti_pattern_guard**: YAML 読込を全 12 箇所横断検索後に着手（`grep -rn "loadBookmarks\|loadYAML\|\.yml" Sources/FocusBMApp/`）
...
# PhaseA: <= 200行 / PhaseB: ファイルごと <= 400行 / PhaseC/E: <= 300行 / PhaseD: <= 200行
```
...
| PhaseC 検出統合で逆方向依存（ProcessProvider→TmuxProvider）解消時に循環/破壊 | high | TerminalDetector へ依存反転集約。抽出/移動/削除の 3 サブタスクに分割しプローブ的に実施。各サブで全 310 維持確認 |
| P2 God 型分割で touched_lines が scope_constraint 逸脱 | high | 抽出/移動/削除の 3 タスク分割を必須化。逸脱検知時 risk=high で即 re-run |
| P5 型リグレッション（AppDelegate 側 YAML 多重読込残存の見落とし） | medium | YAML 読込 12 箇所を着手前に全横断 grep。Phase3 修正と同型を AppDelegate に適用 |
...
| Sources/FocusBMApp/BookmarkRow.swift | 3.4 KB | A, D |
